...
const extraNames: Record<string,Record<string,string>> = {
  ru: { delivery:'Доставка', installation:'Монтаж', other:'Прочее', worktop:'Столешница', plinth:'Цоколь', filler:'Доборы / фальшпанели', decor:'Декоративные боковины', manual:'Нестандартная позиция' },
  en: { delivery:'Delivery', installation:'Installation', other:'Other', worktop:'Worktop', plinth:'Plinth', filler:'Fillers / panels', decor:'Decorative side panels', manual:'Custom item' },
  cs: { delivery:'Doprava', installation:'Montáž', other:'Ostatní', worktop:'Pracovní deska', plinth:'Sokl', filler:'Výplně / krycí panely', decor:'Dekorativní bočnice', manual:'Nestandardní položka' },
  de: { delivery:'Lieferung', installation:'Montage', other:'Sonstiges', worktop:'Arbeitsplatte', plinth:'Sockel', filler:'Blenden / Passleisten', decor:'Dekorseiten', manual:'Sonderposition' },
  pl: { delivery:'Dostawa', installation:'Montaż', other:'Inne', worktop:'Blat', plinth:'Cokół', filler:'Blendki / panele wypełniające', decor:'Boki dekoracyjne', manual:'Pozycja niestandardowa' },
};
...
function extraText(snapshot:QuoteSnapshot, locale:string) {
  ...
  return snapshot.extras.map((extra) => {
...
export async function buildQuotePdf(snapshot:QuoteSnapshot): Promise<Buffer> {
  ...
  const terms: unknown[] = [];
  if (snapshot.terms.depositBps > 0) terms.push({ columns:[{ text:t.deposit, color:'#647169' },{ text:`${(snapshot.terms.depositBps/100).toFixed(2)}% · ${money(snapshot.amounts.depositMinor,snapshot.currency,locale)}`, bold:true, alignment:'right' }], margin:[0,3,0,3] });
  ...
  const docDefinition = {
    pageSize:'A4', pageMargins:[44,46,44,50], info:{ title:`${t.title} ${quoteRef}`, author:snapshot.supplier.tradeName, subject:snapshot.project.name },
    defaultStyle:{ font:'Roboto',fontSize:9.5,color:'#18211d' },
    styles:{ brand:{fontSize:22,bold:true,color:'#196c50'}, title:{fontSize:25,bold:true,margin:[0,26,0,16]}, section:{fontSize:12,bold:true,margin:[0,18,0,8]}, small:{fontSize:8,color:'#647169'} },
    content:[
      { columns:[{width:'*',stack:[{text:snapshot.supplier.tradeName || 'Makster Atelier',style:'brand'},{text:snapshot.supplier.legalName || '',style:'small',margin:[0,4,0,0]}]},{width:210,alignment:'right',stack:[{text:`${t.quote} ${quoteRef}`,bold:true,fontSize:13},{text:`${t.issued}: ${date(snapshot.issuedAt,locale)}`,style:'small',margin:[0,4,0,0]},{text:`${t.valid}: ${date(snapshot.validUntil,locale)}`,style:'small'}]}] },
      { canvas:[{type:'line',x1:0,y1:12,x2:507,y2:12,lineWidth:1.5,lineColor:'#18211d'}] },
      { text:t.title,style:'title' },
      ...(commercialSummary ? [{text:commercialSummary,color:'#196c50',bold:true,margin:[0,-8,0,14]}] : []),
      { columns:[{width:'50%',stack:[{text:t.client,style:'small'},{text:snapshot.client.name,bold:true,margin:[0,4,0,0]},snapshot.client.email?{text:snapshot.client.email}:{},snapshot.client.phone?{text:snapshot.client.phone}:{},snapshot.client.address?{text:snapshot.client.address}:{}]},{width:'50%',stack:[{text:t.project,style:'small'},{text:snapshot.project.name,bold:true,margin:[0,4,0,0]},{text:`Revision ${snapshot.project.revisionNumber}`,style:'small'}]}],columnGap:24 },
      ...
      { margin:[0,22,0,0],columns:[{width:'*',text:''},{width:260,stack:[
        ...(snapshot.commercial && snapshot.commercial.adjustmentMode !== 'none' && snapshot.commercial.adjustmentBps > 0 ? [{columns:[{text:`${snapshot.commercial.variantLabel} · before adjustment`,color:'#647169'},{text:money(snapshot.commercial.listNetMinor,snapshot.currency,locale),alignment:'right'}],margin:[0,4,0,4]}] : []),
      ...
      {text:'Makster Quote · MQ 0.1.12',style:'small',margin:[0,22,0,0]},
    ],
  };
