...
export type QuoteModulePreset = {
  key:string; group:QuoteModuleGroup; name:string; description:string; moduleKey:ModuleKey;
...
const base={...common,heightMm:720,depthMm:560} as const;
const wall={...common,heightMm:720,depthMm:350,stretcherDepthMm:80,shelfSetbackMm:15} as const;
const tall={...common,heightMm:2100,depthMm:560,shelfSetbackMm:20} as const;
...
function wallDoor(widthMm:number):QuoteModulePreset{const doors=widthMm>=700?2:1;return{...wall,key:`wall-door-${widthMm}`,group:'wall-door',name:`Верхний ${widthMm} · ${doors===1?'дверь':'2 двери'}`,description:`Навесной шкаф ${widthMm} мм, высота 720 мм, глубина 350 мм, две полки.`,moduleKey:'w-door',widthMm,drawers:1,doors,shelfCount:2,backMode:'groove'};}
function tallDoor(widthMm:number):QuoteModulePreset{const doors=widthMm>=800?2:1;return{...tall,key:`tall-door-${widthMm}`,group:'tall',name:`Пенал ${widthMm} · ${doors===1?'дверь':'2 двери'}`,description:`Высокий хозяйственный пенал ${widthMm} мм, высота 2100 мм, четыре полки.`,moduleKey:'t-door',widthMm,drawers:1,doors,shelfCount:4,backMode:'groove'};}
...
export const QUOTE_MODULE_PRESETS:QuoteModulePreset[]=[
  ...
  ...[400,450,500,600,800,900].map((width)=>baseDrawer(width,3)),baseDrawer(600,2),baseDrawer(800,2),
