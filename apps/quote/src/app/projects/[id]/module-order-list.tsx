...
type Row={id:string;name:string;quantity:number|string;width_mm:number|string;height_mm:number|string;depth_mm:number|string};
...
function qty(value:number|string){const parsed=Number(value);return Number.isFinite(parsed)?Math.max(1,Math.round(parsed)):1;}

export function ModuleOrderList({projectId,cabinets,locale}:Props){
  if(cabinets.length<2)return null;const m=text[locale];
  return <section className="pageContent compact"><div className="panel"><div className="panelHeader"><div><span className="eyebrow">{m.eyebrow} · MQ 0.2</span><h3>{m.title}</h3><p className="muted">{m.help}</p></div></div><div className="priceList">{cabinets.map((cabinet,index)=><article className="priceRow" key={cabinet.id}><div><span className="pill">{index+1}</span><strong>{cabinet.name}{qty(cabinet.quantity)>1?` × ${qty(cabinet.quantity)}`:''}</strong><small>{Number(cabinet.width_mm)} × {Number(cabinet.height_mm)} × {Number(cabinet.depth_mm)} {m.mm}</small></div><div className="topActions"><form action={moveCabinet}><input type="hidden" name="projectId" value={projectId}/><input type="hidden" name="cabinetId" value={cabinet.id}/><button className="secondary" type="submit" name="direction" value="up" disabled={index===0}>↑ {m.up}</button></form><form action={moveCabinet}><input type="hidden" name="projectId" value={projectId}/><input type="hidden" name="cabinetId" value={cabinet.id}/><button className="secondary" type="submit" name="direction" value="down" disabled={index===cabinets.length-1}>↓ {m.down}</button></form></div></article>)}</div></div></section>;
}
