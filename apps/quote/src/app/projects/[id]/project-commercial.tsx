...
type Props = { projectId:string; clientId:string|null; settings:Record<string,unknown>|null; clients:ClientRow[]; priceBook:PriceRow[]; cabinets:StoredCabinetCost[]; currency:string; targetMarginBps:number; minimumMarginBps:number; overheadBps:number; role:string; quoteHistory:QuoteRow[]; statusEvents:StatusRow[] };
...
export function ProjectCommercial({ projectId, clientId, settings, clients, priceBook, cabinets, currency, targetMarginBps, minimumMarginBps, overheadBps, role, quoteHistory, statusEvents }: Props) {
  ...
  const marginGuardPassed = selected.adjusted.marginBps >= minimumMarginBps;
  const quoteReady = selected.cabinetCount > 0 && selected.incompleteCabinets === 0 && measured.missing.length === 0 && marginGuardPassed;
  ...
  return <section className="pageContent compact"><div className="panel">
    <div className="panelHeader"><div><span className="eyebrow">ИТОГ ПРОЕКТА · MQ 0.1.14</span><h2>Коммерческий расчёт</h2></div><div className="topActions"><Link href={`/projects/${projectId}/quote`} className="secondary linkButton" target="_blank">Живой просмотр</Link>{canPublish ? <form action={publishCommercialQuote}><input type="hidden" name="projectId" value={projectId}/><button className="primary" type="submit" disabled={!quoteReady || !clientId}>Выпустить {COMMERCIAL_VARIANT_LABELS[commercialOptions.selectedVariant]}</button></form> : null}</div></div>
    <div className="padded">
      {!marginGuardPassed ? <div className="notice error"><strong>Profit Guardrail заблокировал выпуск.</strong> Фактическая маржа {(selected.adjusted.marginBps/100).toFixed(2)}% ниже минимально допустимой {(minimumMarginBps/100).toFixed(2)}%. Уменьшите скидку, увеличьте цену или выберите другой вариант.</div>
      : !quoteReady ? <div className="notice warning"><strong>Предложение пока черновое.</strong> Модулей: {selected.cabinetCount}; неполных: {selected.incompleteCabinets}; отделочных позиций без цены: {measured.missing.length}.</div>
      : !clientId ? <div className="notice warning"><strong>Выберите и сохраните клиента.</strong></div>
      : <div className="notice success"><strong>Готово к выпуску.</strong> Вариант {COMMERCIAL_VARIANT_LABELS[commercialOptions.selectedVariant]}, фактическая маржа {(selected.adjusted.marginBps/100).toFixed(2)}% ≥ минимальной {(minimumMarginBps/100).toFixed(2)}%. При выпуске техническое состояние проекта будет зафиксировано отдельной immutable-ревизией.</div>}
      ...
      <section className="metricGrid commercialMetrics">
        ...
        <article className="metricCard"><span>К оплате</span><strong>{formatMinor(selected.adjusted.grossSalesMinor,currency)}</strong><small>фактическая маржа {(selected.adjusted.marginBps/100).toFixed(2)}% · минимум {(minimumMarginBps/100).toFixed(2)}%</small></article>
      </section>
      ...
      <section className="metricGrid" style={{marginBottom:20}}>
        {variants.map((variant) => {
          ...
          const safe = row.adjusted.marginBps >= minimumMarginBps;
          return <article className="metricCard" key={variant} style={active ? {outline:`2px solid ${safe ? '#1f7a5a' : '#a83e3e'}`} : undefined}>
            <span>{active ? 'ВЫБРАННЫЙ ВАРИАНТ' : 'ЦЕНОВОЙ ВАРИАНТ'}</span>
            <strong>{COMMERCIAL_VARIANT_LABELS[variant]}</strong>
            <small>целевая {(commercialOptions.variantMarginsBps[variant]/100).toFixed(2)}% · фактическая {(row.adjusted.marginBps/100).toFixed(2)}% {safe ? '✓' : '⚠ ниже минимума'}</small>
      ...
      <form action={saveProjectCommercial} className="stackForm commercialForm"><input type="hidden" name="projectId" value={projectId}/>
        <div className="formSection"><h3>Ценовые варианты и скидка / надбавка</h3><p className="muted">Base / Standard / Premium сейчас отличаются коммерческой маржой при одинаковом составе мебели. Profit Guardrail не даст выпустить вариант ниже минимальной маржи мастерской.</p>
          ...
          <div className="engineNote"><strong>Минимальная маржа мастерской: {(minimumMarginBps/100).toFixed(2)}%</strong><span>Настраивается владельцем в «Маржа и накладные». Контроль выполняется повторно на сервере непосредственно перед выпуском.</span></div>
        </div>
        ...
        <div className="formActions"><Link href="/price-book" className="secondary linkButton">Прайс-лист</Link><Link href="/settings/pricing" className="secondary linkButton">Маржа и накладные</Link><button type="submit" className="primary">Сохранить условия и варианты</button></div>
      </form>
