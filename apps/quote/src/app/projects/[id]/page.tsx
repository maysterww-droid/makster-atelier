...
export default async function ProjectPage({ params, searchParams }: Props) {
  ...
  const [projectResult, cabinetsResult, priceResult, subscriptionResult, clientsResult] = await Promise.all([
  ...
  return <AppShell organizationName={organization.name} role={role} plan={(subscription?.plan ?? 'free').toUpperCase()}>
    <header className="topbar"><div><span className="eyebrow">{project.project_type.toUpperCase()} · {project.status.toUpperCase()}</span><h1>{project.name}</h1></div><div className="topActions"><Link href="/" className="textLink">← Проекты</Link><Link href="/price-book" className="secondary linkButton">Прайс-лист</Link><Link href={`/projects/${project.id}/quote`} className="primary linkButton" target="_blank">Предложение / PDF</Link></div></header>
    {query.error ? <div className="pageContent compact"><div className="notice error">Не удалось сохранить изменения ({query.error}). Проверьте данные и попробуйте снова.</div></div> : null}
    ...
    <ProjectCommercial projectId={project.id} clientId={project.client_id} settings={project.settings as Record<string, unknown>} clients={clientsResult.data ?? []} priceBook={(priceBook ?? []) as never[]} cabinets={(cabinets ?? []) as never[]} currency={project.currency} targetMarginBps={targetMarginBps} overheadBps={overheadBps}/>
