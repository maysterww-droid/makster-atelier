...
const labels: Record<string, Labels> = {
  ru: { document:'Коммерческое предложение',draft:'ЧЕРНОВИК',client:'Клиент',noClient:'Клиент не указан',project:'Проект',date:'Дата',validUntil:'Действительно до',modules:'Состав проекта',module:'Модуль',dimensions:'Размеры',included:'Включено в расчёт',net:'Итого без НДС',tax:'НДС / налог',total:'Итого к оплате',note:'Примечание',supplier:'Исполнитель',print:'Печать / сохранить PDF',back:'← Вернуться к проекту',delivery:'Доставка',installation:'Монтаж',other:'Прочее' },
  en: { document:'Quotation',draft:'DRAFT',client:'Client',noClient:'Client not specified',project:'Project',date:'Date',validUntil:'Valid until',modules:'Project scope',module:'Module',dimensions:'Dimensions',included:'Included in calculation',net:'Total excl. tax',tax:'Tax',total:'Total payable',note:'Note',supplier:'Supplier',print:'Print / Save PDF',back:'← Back to project',delivery:'Delivery',installation:'Installation',other:'Other' },
  cs: { document:'Cenová nabídka',draft:'NÁVRH',client:'Klient',noClient:'Klient neuveden',project:'Projekt',date:'Datum',validUntil:'Platnost do',modules:'Rozsah projektu',module:'Modul',dimensions:'Rozměry',included:'Zahrnuto v kalkulaci',net:'Celkem bez DPH',tax:'DPH / daň',total:'Celkem k úhradě',note:'Poznámka',supplier:'Dodavatel',print:'Tisk / uložit PDF',back:'← Zpět k projektu',delivery:'Doprava',installation:'Montáž',other:'Ostatní' },
  de: { document:'Angebot',draft:'ENTWURF',client:'Kunde',noClient:'Kein Kunde angegeben',project:'Projekt',date:'Datum',validUntil:'Gültig bis',modules:'Projektumfang',module:'Modul',dimensions:'Maße',included:'In der Kalkulation enthalten',net:'Summe netto',tax:'MwSt. / Steuer',total:'Gesamtbetrag',note:'Hinweis',supplier:'Auftragnehmer',print:'Drucken / als PDF speichern',back:'← Zurück zum Projekt',delivery:'Lieferung',installation:'Montage',other:'Sonstiges' },
  pl: { document:'Oferta',draft:'WERSJA ROBOCZA',client:'Klient',noClient:'Nie wskazano klienta',project:'Projekt',date:'Data',validUntil:'Ważna do',modules:'Zakres projektu',module:'Moduł',dimensions:'Wymiary',included:'Uwzględniono w kalkulacji',net:'Razem netto',tax:'VAT / podatek',total:'Razem do zapłaty',note:'Uwagi',supplier:'Wykonawca',print:'Drukuj / zapisz PDF',back:'← Wróć do projektu',delivery:'Dostawa',installation:'Montaż',other:'Inne' },
};
...
export default async function QuotePage({ params }: Props) {
  ...
  const [{ data: project, error: projectError }, { data: cabinets, error: cabinetsError }, { data: priceBook, error: priceError }] = await Promise.all([
    ...
    supabase.from('quote_price_book_items').select('id, category, name, purchase_price_minor').eq('organization_id', organization.id).eq('active', true),
  ]);
  ...
  const selected = (itemId: string) => priceRows.find((item) => item.id === itemId);
  const delivery = selected(commercial.deliveryItemId);
  const installation = selected(commercial.installationItemId);
  const other = selected(commercial.otherItemId);
  ...
  return <main className="quoteDocument">
    ...
    <header className="quoteHead"><div className="quoteBrand">Makster<small>{organization.name}</small></div><div className="quoteNumber"><strong>MQ-{project.id.slice(0,8).toUpperCase()}</strong><span>{t.date}: {formatter.format(issued)}</span><span>{t.validUntil}: {formatter.format(validUntil)}</span></div></header>
    ...
    <footer className="quoteFooter"><strong>{t.supplier}: {organization.name}</strong><div className="quoteContact">Makster Quote · MQ 0.1.3</div></footer>
  </main>;
}
