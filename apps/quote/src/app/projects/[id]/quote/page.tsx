...
type Labels = {
  ...
  modules: string; module: string; dimensions: string; quantity: string; included: string; net: string; tax: string; total: string; note: string;
  supplier: string; print: string; back: string; delivery: string; installation: string; other: string;
};

const labels: Record<string, Labels> = {
  ru: { document:'Коммерческое предложение · живой просмотр',draft:'ЧЕРНОВИК',client:'Клиент',noClient:'Клиент не указан',project:'Проект',date:'Дата',validUntil:'Действительно до',modules:'Состав проекта',module:'Модуль',dimensions:'Размеры',quantity:'Кол-во',included:'Включено в расчёт',net:'Итого без НДС',tax:'НДС / налог',total:'Итого к оплате',note:'Примечание',supplier:'Исполнитель',print:'Печать / сохранить PDF',back:'← Вернуться к проекту',delivery:'Доставка',installation:'Монтаж',other:'Прочее' },
  en: { document:'Quotation · live preview',draft:'DRAFT',client:'Client',noClient:'Client not specified',project:'Project',date:'Date',validUntil:'Valid until',modules:'Project scope',module:'Module',dimensions:'Dimensions',quantity:'Qty',included:'Included in calculation',net:'Total excl. tax',tax:'Tax',total:'Total payable',note:'Note',supplier:'Supplier',print:'Print / Save PDF',back:'← Back to project',delivery:'Delivery',installation:'Installation',other:'Other' },
  cs: { document:'Cenová nabídka · živý náhled',draft:'NÁVRH',client:'Klient',noClient:'Klient neuveden',project:'Projekt',date:'Datum',validUntil:'Platnost do',modules:'Rozsah projektu',module:'Modul',dimensions:'Rozměry',quantity:'Počet',included:'Zahrnuto v kalkulaci',net:'Celkem bez DPH',tax:'DPH / daň',total:'Celkem k úhradě',note:'Poznámka',supplier:'Dodavatel',print:'Tisk / uložit PDF',back:'← Zpět k projektu',delivery:'Doprava',installation:'Montáž',other:'Ostatní' },
  de: { document:'Angebot · Live-Vorschau',draft:'ENTWURF',client:'Kunde',noClient:'Kein Kunde angegeben',project:'Projekt',date:'Datum',validUntil:'Gültig bis',modules:'Projektumfang',module:'Modul',dimensions:'Maße',quantity:'Anzahl',included:'In der Kalkulation enthalten',net:'Summe netto',tax:'MwSt. / Steuer',total:'Gesamtbetrag',note:'Hinweis',supplier:'Auftragnehmer',print:'Drucken / als PDF speichern',back:'← Zurück zum Projekt',delivery:'Lieferung',installation:'Montage',other:'Sonstiges' },
  pl: { document:'Oferta · podgląd na żywo',draft:'WERSJA ROBOCZA',client:'Klient',noClient:'Nie wskazano klienta',project:'Projekt',date:'Data',validUntil:'Ważna do',modules:'Zakres projektu',module:'Moduł',dimensions:'Wymiary',quantity:'Ilość',included:'Uwzględniono w kalkulacji',net:'Razem netto',tax:'VAT / podatek',total:'Razem do zapłaty',note:'Uwagi',supplier:'Wykonawca',print:'Drukuj / zapisz PDF',back:'← Wróć do projektu',delivery:'Dostawa',installation:'Montaż',other:'Inne' },
};
...
export default async function QuotePage({ params }: Props) {
  ...
  return <main className="quoteDocument">
    ...
    <h2>{t.modules}</h2>
    <table className="quoteTable"><thead><tr><th>{t.module}</th><th>{t.dimensions}</th><th>{t.quantity}</th></tr></thead><tbody>{(cabinets ?? []).map((cabinet) => <tr key={cabinet.id}><td>{cabinet.name}</td><td>{Number(cabinet.width_mm)} × {Number(cabinet.height_mm)} × {Number(cabinet.depth_mm)} mm</td><td>{Number(cabinet.quantity ?? 1)}</td></tr>)}</tbody></table>
    ...
    <footer className="quoteFooter"><strong>{t.supplier}: {brand.tradeName}</strong><div className="quoteContact">{[brand.address, brand.email, brand.phone, brand.website].filter(Boolean).join(' · ')}</div><div className="quoteContact">Makster Quote · LIVE preview · MQ 0.1.7</div></footer>
  </main>;
}
