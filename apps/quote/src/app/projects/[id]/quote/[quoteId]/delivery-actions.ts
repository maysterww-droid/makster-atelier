...
export type DeliveryActionState={status:'idle'|'success'|'error';message:string;publicUrl?:string;linkId?:string;recipient?:string;warning?:string};
const deliveryRoles=new Set(['owner','admin','sales','technologist']);
function text(formData:FormData,name:string,max=500){return String(formData.get(name)??'').trim().slice(0,max);}
function tokenHash(token:string){return createHash('sha256').update(token).digest('hex');}
function emailLooksValid(value:string){return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)&&value.length<=320;}
async function publicBaseUrl(){const configured=process.env.NEXT_PUBLIC_APP_URL?.trim().replace(/\/$/,'');if(configured)return configured;const h=await headers();const host=h.get('x-forwarded-host')??h.get('host');const proto=h.get('x-forwarded-proto')??(host?.includes('localhost')?'http':'https');if(host)return `${proto}://${host}`;const vercel=process.env.VERCEL_URL?.trim();if(vercel)return `https://${vercel.replace(/\/$/,'')}`;throw new Error('public-url-unavailable');}
...
export async function sendQuoteEmail(_previous:DeliveryActionState,formData:FormData):Promise<DeliveryActionState>{
  const localeUi=await getInterfaceLocale();const m=getDeliveryMessages(localeUi);const projectId=text(formData,'projectId',80);const quoteId=text(formData,'quoteId',80);const recipient=text(formData,'recipientEmail',320);const apiKey=process.env.RESEND_API_KEY?.trim();const from=process.env.QUOTE_EMAIL_FROM?.trim();
...
export async function revokeClientLink(formData:FormData){const projectId=text(formData,'projectId',80);const quoteId=text(formData,'quoteId',80);const linkId=text(formData,'linkId',80);const {supabase,role}=await requireWorkspace();if(!deliveryRoles.has(role))return;await supabase.rpc('quote_revoke_client_access_link',{p_link_id:linkId});revalidatePath(`/projects/${projectId}/quote/${quoteId}`);}
