...
function extraText(extra: { name:string; quantity?:number; unit?:'m'|'m2'|'job' }) {
  ...
  return `${extra.name} · ${extra.quantity.toFixed(2)} ${extra.unit === 'm2' ? 'м²' : 'м'}`;
}
...
export default async function PublishedQuotePage({ params, searchParams }: Props) {
  ...
  return <main className="quoteDocument">
    ...
    <section className="title"><h1>Коммерческое предложение</h1><span className="status">{statusName[latest] ?? latest}</span>{snapshot.commercial ? <span className="status">{snapshot.commercial.variantLabel}</span> : null}{adjustment ? <span className="status">{adjustment}</span> : null}</section>
    <section className="info"><div><h3>Клиент</h3><strong>{snapshot.client.name}</strong>{snapshot.client.email ? <span>{snapshot.client.email}</span> : null}{snapshot.client.phone ? <span>{snapshot.client.phone}</span> : null}{snapshot.client.address ? <span>{snapshot.client.address}</span> : null}</div><div><h3>Проект</h3><strong>{snapshot.project.name}</strong><span>Зафиксированная ревизия {snapshot.project.revisionNumber}</span><span>Версия предложения {quote.quote_version}</span></div></section>
    ...
    {snapshot.extras.length ? <p><strong>Включено:</strong> {snapshot.extras.map(extraText).join(' · ')}</p> : null}
    <section className="totals">{snapshot.commercial && snapshot.commercial.adjustmentMode !== 'none' && snapshot.commercial.adjustmentBps > 0 ? <div><span>{snapshot.commercial.variantLabel} · до корректировки</span><strong>{formatMinor(minorFromUnknown(snapshot.commercial.listNetMinor), snapshot.currency)}</strong></div> : null}<div><span>Итого без НДС</span><strong>{formatMinor(minorFromUnknown(snapshot.amounts.netMinor), snapshot.currency)}</strong></div>{snapshot.amounts.taxBps > 0 ? <div><span>НДС / налог {(snapshot.amounts.taxBps/100).toFixed(2)}%</span><strong>{formatMinor(minorFromUnknown(snapshot.amounts.taxMinor), snapshot.currency)}</strong></div> : null}<div className="grand"><span>К оплате</span><strong>{formatMinor(minorFromUnknown(snapshot.amounts.totalMinor), snapshot.currency)}</strong></div></section>
