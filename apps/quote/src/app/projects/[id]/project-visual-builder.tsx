...
export function ProjectVisualBuilder({cabinets,currency,locale}:Props){
  ...
  const totalWidth=sceneItems.reduce((sum,item)=>sum+dimension(item.row.width_mm,600),0);
  ...
  return <>
    <section className={styles.scenePanel}>
      ...
      <div className={styles.sceneViewport}>
        {sceneItems.length?<div className={styles.sceneLine}>{sceneItems.map(({sceneId,row,index})=>{
          const width=dimension(row.width_mm,600); const mode=level(row); const px=Math.max(64,Math.min(160,width/5.8));
          return <button key={sceneId} type="button" onClick={()=>setSelectedId(row.id)} className={`${styles.sceneUnit} ${row.id===selected?.id?styles.sceneUnitSelected:''} ${mode==='tall'?styles.tallUnit:mode==='wall'?styles.wallUnit:styles.baseUnit}`} style={{width:px}} aria-label={`${row.name} ${index+1}`}>
            <ModuleSchematic className={styles.sceneSvg} moduleKey={row.module_key} name={row.name} widthMm={row.width_mm} heightMm={row.height_mm}/><span className={styles.unitLabel}>{width} mm</span>
          </button>;
