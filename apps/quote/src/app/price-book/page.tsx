...
const importErrors: Record<string,string> = {
  ...
  'import-row':'В одной из строк неверная категория, единица, валюта, название или цена.', 'import-price':'В одной из строк неверный формат цены.',
...
export default async function PriceBookPage({ searchParams }: Props) {
  ...
  return <AppShell organizationName={organization.name} role={role} plan={(subscription?.plan ?? 'free').toUpperCase()}>
    ...
    <div className="pageContent">
      ...
      <div className="notice warning"><strong>Цена привязана к валюте.</strong> Проект в EUR использует только EUR-позиции, проект в CZK — только CZK-позиции. Автоматического пересчёта курса в 0.1.15 нет: так себестоимость не зависит от случайного курса.</div>
      ...
      {canManage ? <section className="panel formPanel">
        ...
        <form action={importPriceBookCsv} className="stackForm padded"><label>CSV-файл<input name="file" type="file" accept=".csv,text/csv,text/plain" required /></label><div className="engineNote"><strong>Обязательные колонки</strong><span>category, name, unit, price</span><span>Необязательная колонка currency: CZK / EUR / PLN / USD. Если её нет, используется {organization.currency}.</span></div><button type="submit" className="secondary">Импортировать CSV</button></form>
      </section> : null}
      ...
      <div className="twoColumnPage">
        <section className="panel formPanel">
          ...
          <form action={addPriceBookItem} className="stackForm padded">
            <div className="formGrid3"><label>Категория<select name="category" defaultValue="board">{categories.map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Единица<select name="unit" defaultValue="sheet">{units.map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Валюта<select name="currency" defaultValue={organization.currency}>{currencies.map((currency) => <option key={currency} value={currency}>{currency}</option>)}</select></label></div>
            ...
            <div className="formGrid2"><label>Закупочная цена<input name="price" inputMode="decimal" required /></label><label>Толщина, мм<input name="thicknessMm" type="number" min="0" step="0.1" /></label></div>
