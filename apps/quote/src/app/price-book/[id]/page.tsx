...
export default async function EditPriceBookItemPage({ params, searchParams }: Props) {
  ...
  const [{ data: item, error }, { data: subscription }] = await Promise.all([
    supabase
      ...
      .select('id, category, name, manufacturer, sku, unit, currency, purchase_price_minor, parameters_json, active')
      .eq('id', id)
  ...
  return (
    <AppShell organizationName={organization.name} role={role} plan={(subscription?.plan ?? 'free').toUpperCase()}>
      <header className="topbar">
        <div><span className="eyebrow">PRICE BOOK · EDIT</span><h1>Изменить цену</h1></div>
        <Link href="/price-book" className="textLink">← Прайс-лист</Link>
      </header>
      <div className="pageContent narrow">
        ...
        <section className="panel formPanel">
          <div className="panelHeader"><div><h2>{item.name}</h2><p className="muted">Изменение цены влияет только на будущие расчёты и текущие черновики. Уже выпущенные предложения остаются неизменяемыми.</p></div></div>
          {canManage ? <form action={updatePriceBookItem} className="stackForm padded">
            <input type="hidden" name="itemId" value={item.id}/>
            <div className="formGrid2">
              <label>Категория<select name="category" defaultValue={item.category}><option value="board">Плита / задняя стенка</option><option value="front">Фасад</option><option value="edge">Кромка</option><option value="hardware">Фурнитура</option><option value="labour">Работа</option><option value="operation">Операция</option><option value="delivery">Доставка</option><option value="installation">Монтаж</option><option value="overhead">Накладные</option><option value="other">Прочее</option></select></label>
              <label>Единица<select name="unit" defaultValue={item.unit}><option value="sheet">Лист</option><option value="m2">м²</option><option value="m">метр</option><option value="pcs">штука</option><option value="set">комплект</option><option value="hour">час</option><option value="job">заказ</option></select></label>
            </div>
            ...
            <div className="formGrid2"><label>Закупочная цена, {organization.currency}<input name="price" inputMode="decimal" defaultValue={priceInput(item.purchase_price_minor)} required /></label><label>Толщина материала, мм<input name="thicknessMm" type="number" min="0" step="0.1" defaultValue={Number(parameters.thicknessMm ?? 0) || ''} /></label></div>
