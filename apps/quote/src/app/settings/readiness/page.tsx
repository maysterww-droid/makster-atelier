...
import { billingStatusLabel, getSettingsMessages } from '@/lib/i18n-settings';
...
export default async function ReadinessPage(){
  ...
  const overall=overallReadiness(checks); const subscription=subscriptionResult.data; const readyCount=checks.filter((check)=>check.state==='ready').length; const subscriptionStatus=billingStatusLabel(locale,subscription?.status??'inactive');
  const billingProvider=subscription?.provider==='lemonsqueezy'?'Lemon Squeezy':subscription?.provider??m.notConnected;
  const billingMode=subscription?.test_mode?runtime.modeTest:runtime.modeLiveOrUnset;
