...
import { findReadyModulePreview, type ReadyModuleLibraryRow } from '@/lib/asset-bridge';
...
export default async function LibraryPage({searchParams}:Props){
  ...
  const [{data:projects,error:projectError},{data:subscription},{data:prefs,error:prefsError},{data:templates,error:templatesError},{data:readyModules,error:readyModulesError}]=await Promise.all([
    ...
    supabase.from('ready_module_library').select('module_code,template_code,version,release_status,width_mm,depth_mm,body_height_mm,preview_bucket,preview_path,preview_mime_type'),
  ]);
  if(projectError)throw new Error(`Failed to load projects: ${projectError.message}`);if(prefsError)throw new Error(`Failed to load library preferences: ${prefsError.message}`);if(templatesError)throw new Error(`Failed to load workshop standards: ${templatesError.message}`);if(readyModulesError)throw new Error(`Failed to load Makster READY library: ${readyModulesError.message}`);
  const readyLibrary=(readyModules??[]) as ReadyModuleLibraryRow[];
  ...
  const bridgePreview=(preset:(typeof ALL_QUOTE_MODULE_PRESETS)[number])=>{const asset=findReadyModulePreview(preset,readyLibrary);if(!asset)return null;const {data}=supabase.storage.from(asset.bucket).getPublicUrl(asset.path);return {url:data.publicUrl,version:asset.version};};

  const compactPreset=(preset:(typeof ALL_QUOTE_MODULE_PRESETS)[number])=>{const localized=localizePreset(locale,preset);const preview=bridgePreview(preset);return <article className="metricCard" key={preset.key}><ModuleThumbnail preset={preset} previewUrl={preview?.url} bridgeVersion={preview?.version}/><strong style={{fontSize:16,marginTop:8}}>{localized.name}</strong><small>{preset.widthMm} × {preset.heightMm} × {preset.depthMm} {m.mm}</small>{selectedProject?<form action={addPresetToProject} className="stackForm"><input type="hidden" name="projectId" value={selectedProject.id}/><input type="hidden" name="presetKey" value={preset.key}/><input type="hidden" name="quantity" value="1"/><button className="secondary wide" type="submit">+ {m.addTo} {selectedProject.name}</button></form>:null}</article>;};
