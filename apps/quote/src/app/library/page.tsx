...
import { QUOTE_MODULE_PRESETS, quoteModulePreset, type QuoteModuleFamily } from '@/lib/module-presets';
...
export default async function LibraryPage({searchParams}:Props){
  ...
  const favoritePresets=QUOTE_MODULE_PRESETS.filter((preset)=>favoriteKeys.has(preset.key)).slice(0,8);
  const recentPresets=[...(prefs??[])].filter((row)=>row.last_used_at).sort((a,b)=>String(b.last_used_at).localeCompare(String(a.last_used_at))).map((row)=>quoteModulePreset(row.preset_key)).filter((preset):preset is NonNullable<typeof preset>=>Boolean(preset)).slice(0,8);
