-- R34: atomically apply Dream Planner updates to the same Quote project.
alter table public.quote_planner_imports add column if not exists latest_source_hash text;
alter table public.quote_planner_imports add column if not exists sync_count integer not null default 1 check(sync_count>=1);
alter table public.quote_planner_imports add column if not exists last_synced_at timestamptz not null default now();
create index if not exists quote_planner_imports_source_idx on public.quote_planner_imports(organization_id,source_project_id,updated_at desc);
...
declare v_revision_id uuid;v_revision_number integer;v_module_count integer:=0;v_ready_count integer:=0;
begin
  ...
  select coalesce(max(revision_number),0)+1 into v_revision_number from public.project_revisions where organization_id=p_organization_id and project_id=p_project_id;
  insert into public.project_revisions(organization_id,project_id,revision_number,schema_version,snapshot_json,change_set_json,source)
  values(p_organization_id,p_project_id,v_revision_number,'1.0.0',coalesce(p_snapshot,'{}'::jsonb),jsonb_build_array(jsonb_build_object('type','planner_sync','handoffId',p_handoff_id,'sourceHash',p_source_hash,'syncCount',p_sync_count)),'import') returning id into v_revision_id;
  update public.projects set name=left(coalesce(nullif(p_project_name,''),name),200),currency=coalesce(nullif(p_currency,''),currency),settings=coalesce(p_settings,'{}'::jsonb),current_revision_id=v_revision_id,updated_at=now() where id=p_project_id and organization_id=p_organization_id;
